...
const ContextState = (props) => {
  /*
        Plain Reducer
    */
  const [statePlainReducer, dispatchPlainReducer] = useReducer(
    PlainReducer.PlainReducer,
    PlainReducer.initialState
  );

  const handleDispatchTrue = () => {
    //    dispatchPlainReducer(type: "SUCCESS")
    //    dispatchPlainReducer(ACTIONS.SUCCESS)
    dispatchPlainReducer(ACTIONS.success());
  };

  const handleDispatchFalse = () => {
    //     dispatchPlainReducer(type: "FAILURE")
    //    dispatchPlainReducer(ACTIONS.FAILURE)
    dispatchPlainReducer(ACTIONS.failure());
  };
  ...
  return (
    <div>
      <Context.Provider
        value={{
          stateProp1: statePlainReducer.stateprop1,
